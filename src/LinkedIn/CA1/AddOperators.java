...
public class AddOperators {
    ...
    private void helper(char[] nums, StringBuilder sb, int index, long prevSum, long prevNum, int target, List<String> res) {
        ...
        for (int i = index; i < nums.length; i++) {
            ...
            if (index == 0) {
                ...
                helper(nums, sb.append("+").append(cur), i + 1, prevSum + curNum, curNum, target, res);
                sb.setLength(length);
                helper(nums, sb.append("-").append(cur), i + 1, prevSum - curNum, -curNum, target, res);
                sb.setLength(length);
                helper(nums, sb.append("*").append(cur), i + 1, prevSum - prevNum + prevNum * curNum, prevNum * curNum, target, res);
                sb.setLength(length);
                if (curNum != 0) {
                    helper(nums, sb.append("-").append(cur), i + 1, prevSum - prevNum + prevNum / curNum, prevNum / curNum, target, res);
                    sb.setLength(length);
