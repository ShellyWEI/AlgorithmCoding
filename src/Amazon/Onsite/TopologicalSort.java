...
public class TopologicalSort {
    ...
    public int[] findOrder(int numCourses, int[][] prerequisites) {
        ...
        // build graph, calculating inDegree and adding neighbors
        for (int[] edge : prerequisites) {
            GraphNode parent = map.get(edge[1]);
            GraphNode child = map.get(edge[0]);
            if (parent == null) {
                parent = new GraphNode(edge[1]);
                map.put(edge[1], parent);
            }
            if (child == null) {
                child = new GraphNode(edge[0]);
                map.put(edge[0], child);
            }
            child.inDegree++;
            parent.neis.add(child);
        }
        // find entry where inDegree == 0, and those
        // courses don't have prerequisites
        ...
        int index = 0;
        while (!queue.isEmpty()) {
            GraphNode curCourse = queue.poll();
            res[index++] = curCourse.val;
            List<GraphNode> neis = curCourse.neis;
        ...
        // if can't traverse all, means cycle
        if (index < numCourses) {
            return new int[0];
        }
        return res;
    ...
    public boolean canFinish(int numCourses, int[][] prerequisites) {
        // we can't use 0 or 1 because there can be more than one connection
        int[] visited = new int[numCourses]; // non visiting point
        //Map<Integer, List<Integer>> neiMap = buildMap(prerequisites);
        List[] neis = new List[numCourses];
        int[] inDegree = new int[numCourses];
        for (int[] edge : prerequisites) {
            int parent = edge[1];
            int child = edge[0];
            inDegree[child]++;
            List<Integer> cur = neis[parent];
            if (cur == null) {
                cur = new ArrayList<>();
                neis[parent] = cur;
            }
        ...
        for (int i = 0; i < numCourses; i++) {
            if (inDegree[i] == 0 && visited[i] == 0) {
                if (!dfs(neis, i, visited)) {
                    return false;
                }
            }
        }
        // check if some courses never take, inside those will be loop
        for (int i = 0; i < numCourses; i++) {
            if (visited[i] == 0) {
                return false;
            }
    ...
    private boolean dfs(List[] neis, int cur, int[] visited) {
        // there is a cycle
        if (visited[cur] == 1) {
            return false;
        }
        visited[cur] = 1; // current visiting
        // reach end of path
        List<Integer> curNeis = neis[cur];
        if (curNeis == null) {
            visited[cur] = 2; // visited
            return true;
        }
        for (Integer nei : curNeis) {
            if (!dfs(neis, nei, visited)) {
                return false;
            }
        }
        visited[cur] = 2; // visited
